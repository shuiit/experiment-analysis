function h = daviolinplot(Y,xdata_fly,norm_vec,varargin)
...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        if strcmp(confs.smoothing,'default')
            [f,xi] = ksdensity(data_vals,'Support',[0,180],'BoundaryCorrection','reflection');
        ...
        f = confs.violinwidth.*(f/norm_vec(k))*(xdata_fly(2)-xdata_fly(1) - 10);
        ...
        elseif strcmp(confs.violin,'half')   
            if confs.box==3
                ...
                h.ds(k,g) = fill(f+xdata_fly(k),xi,confs.colors(g,:));
            end 
        elseif strcmp(confs.violin,'half2')   
            if confs.box==3
        ...
        if confs.box~=0
            ...
            if strcmp(confs.boxcolors,'same')
                ...
                h.bx(k,g) = fill([Xx(:,1)' Xx(1,:) Xx(:,2)' Xx(2,[2,1])],...
                     [Yy(:,1)' Yy(1,:) Yy(:,2)' Yy(2,:)],confs.colors);
            end
            ...
            if confs.whiskers==1
                ...
                whi_xcor = mean(Xx')';
                % plot whiskers
                h.wh(k,g,:) = plot(whi_xcor,whi_ycor(:,1,k),'k-',... 
                    whi_xcor,whi_ycor(:,2,k),'k-',... 
                    'LineWidth',1.5);                              
            end             
        end
        ...
        if confs.scatter~=0 
            ...
            xdata = xdata*0 + xdata_fly(k) - confs.scatter;
        ...
        elseif confs.scatter==2 
            if confs.box==3
                ...
                xdata = xdata - box_width/4;
                        ...
                        xdata = xdata*0 + xdata_fly(k) + xdata_fly(k)*0.5;
...
if ~isempty(confs.legend) && iscell(confs.legend) == 0
    h.lg = legend(h.ds(1,:),confs.legend,'Box','off');
end

if iscell(confs.legend) == 1
    p = get(gca);   
    p.Legend.String = confs.legend
end
