...
class Experiment():
    def __init__(self,loadir,exp_name, movie_name_list = False, movie_length = 2000):    
        ...
        self.exp_dict = {mov : ManipulatedMovie(self.experiment,mov,pertubation = self.pertubation) for mov in self.mov_names if self.del_initial_tim_and_length(self.experiment[mov],movie_length,time_idx) !=True}
    ...
    def mean_props_movies(self,prop1,prop2,wing_body,header_name, mov = False):
        mov_names = self.mov_names if mov == False else mov
