...
% set(gca,'fontsize',20)
        ...
        function h = violin_plot(obj,insect,prop_name,time_to_violin,label_y,color,fps,varargin)
            parser = inputParser;
            addParameter(parser,'xlabel','time [ms]'); % number of camera pointing in Z lab axis
            parse(parser, varargin{:})
            idx_to_violin = find(ismembertol(insect.time_vec,time_to_violin,1/fps -1/fps/2 ,'DataScale', 1))
            x_names = insect.time_vec(idx_to_violin)
            
            prop = insect.get_prop(prop_name);
            prop_time = prop(idx_to_violin,:);
            h = daviolinplot(prop_time',x_names,'colors',color,'box',0,'whiskers',0,'violinalpha',0.5,'bins',15,'outliers',0);
            xlabel(parser.Results.xlabel)
            ylabel(label_y)
        end
