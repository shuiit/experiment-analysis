...
class PreProcess:
    ...
    def __init__(self,exp_dir,exp_name, smoothing_config):
    ...
    def manual_clip_frames(self,mov,ax,ax_twin):
        ...
        plt.pause(0.00001)
        x = plt.ginput(2)
        if len(x) > 0: self.save_mov_to_dict(mov,x)

    def save_mov_to_dict(self,mov,x):
        ...
        self.exp_dict[f'mov{mov}'] = Movie(data_list,mov)
